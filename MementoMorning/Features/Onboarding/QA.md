...
- [x] **プログレス表示**: 起床の質問から メメント・モリ までの 8 画面だけ、画面上部 (セーフエリア直下) に夜明け色の 1pt のラインが 1/8 → 8/8 と伸びる。コンセプト・許可以降 (明日への約束を含む)・ペイウォールには出ない
  - 自動化: manual（描画の目視確認）
- [x] **明日への約束の長押し**: アラーム設定の保存後、見出し「明日の自分に、ひとつの約束」と宣誓文「明日の 7:00、私は目を覚まし、自分と向き合い、答えます。」(設定した時刻が入る。設定時刻より前に保存して最初のアラームが当日に鳴る場合は「今日の自分に、ひとつの約束」「今日の 7:00、…」になる。この分岐は MementoMorningTests/OnboardingMorningsTests.swift の PledgeFiresTodayTests がカバー済みで、画面では明日の分岐だけ確認した) が表示され、リング (onboarding_pledge_hold) の下に「長押しで約束する」が出る。リングを 1.2 秒以上長押しすると夜明け色のリングが満ちて案内が「約束しました」に変わり、約 0.8 秒後に儀式のサマリーへフェードで進む。1.2 秒未満で指を離すとリングは戻り、進まない。「つづける」などのボタンは無く、長押し以外で進む導線は無い
...
### **明日への約束の長押し**: アラーム設定の保存後、見出し「明日の自分に、ひとつの約束」と宣誓文「明日の 7:00、私は目を覚まし、自分と向き合い、答えます。」(設定した時刻が入る。設定時刻より前に保存して最初のアラームが当日に鳴る場合は「今日の自分に、ひとつの約束」「今日の 7:00、…」になる。この分岐は MementoMorningTests/OnboardingMorningsTests.swift の PledgeFiresTodayTests がカバー済みで、画面では明日の分岐だけ確認した) が表示され、リング (onboarding_pledge_hold) の下に「長押しで約束する」が出る。リングを 1.2 秒以上長押しすると夜明け色のリングが満ちて案内が「約束しました」に変わり、約 0.8 秒後に儀式のサマリーへフェードで進む。1.2 秒未満で指を離すとリングは戻り、進まない。「つづける」などのボタンは無く、長押し以外で進む導線は無い
