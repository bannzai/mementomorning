...
| ID | 期待挙動 | 対応項目 |
|----|---------|---------|
| S1 | アラーム停止 → 質問画面 → 回答 → 以降アラームが鳴らない、の一連が通る | 回答成立で当日アラームが鳴らない / 回答成立で追撃・バックアップ含む全キャンセル (実機) |
...
- [x] **回答成立で当日アラームが鳴らない**: 回答が成立すると当日のアラームがキャンセルされ、設定した時刻を過ぎてもアラームが発火しない
...
- [ ] **回答成立で追撃・バックアップ含む全キャンセル (実機)**: 追撃アラームが登録された状態で回答が成立すると、バックアップ・追撃を含む当日の残アラームが全キャンセルされる
  - 自動化: manual（実機のみ）
  - ⏭️ 実機 QA へ: 追撃の登録自体がシミュレータで走らない (StopAlarmIntent.perform() 非実行) ため、追撃が存在する状態のキャンセルはシミュレータで確認できない (元は「回答成立で全アラームキャンセル」の一部だったが、未確認のまま項目全体を通過扱いにしないため分割)
...
### **回答成立で当日アラームが鳴らない**: 回答が成立すると当日のアラームがキャンセルされ、設定した時刻を過ぎてもアラームが発火しない
