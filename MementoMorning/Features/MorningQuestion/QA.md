...
- [x] **アラーム OFF 中は提示しない**: アラームを OFF にすると、発火記録が残っていても朝の問いが提示されない (提示中でも閉じる)
  - 自動化: manual（画面上の挙動の目視確認。提示判定のロジックは MementoMorningTests/MorningQuestionGateTests.swift がカバー済み）
  - 確認範囲: 「アラーム OFF + 発火記録あり + 未回答で提示されない」ことを確認した。括弧内の「提示中でも閉じる」は未確認 — 朝の問いは fullScreenCover で、提示中はホームの pill トグルにもアラーム設定にも到達できず、UI 操作でアラームを OFF にする手段が無い
...
- [x] **回答成立で全アラームキャンセル**: 回答が成立すると当日の残アラーム (バックアップ・追撃含む) が全キャンセルされ、以降アラームが鳴らない
  ...
  - 確認範囲: シミュレータで「回答成立後、設定した時刻を過ぎてもアラームが発火しない」ことを確認した。追撃アラームぶんのキャンセルは、追撃の登録自体がシミュレータで走らない (StopAlarmIntent.perform() 非実行) ため実機 QA に残る
