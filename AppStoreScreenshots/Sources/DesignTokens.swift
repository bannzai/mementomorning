import SwiftUI

// デザイントークンの複製。SSOT は MementoMorning/Shared/DesignSystem/DesignSystem.swift
// (原典: design_handoff_memento_morning/README.md の Design Tokens)。
// 本番ソースを AppStoreScreenshots ターゲットに含めると xcodebuild が multiple groups の
// malformed project 警告を出すため、スクショに必要なトークンだけをここに複製する。
// 本番側のトークンを変更した時はこのファイルにも反映する

extension Color {
    /// 背景 (墨)。#0B0C0E
    static let ink = Color(red: 0x0B / 255, green: 0x0C / 255, blue: 0x0E / 255)
    /// 前景 (温白)。#E9E7E2。二次テキストは opacity 0.55 / 三次 0.40 / 微弱 0.30–0.35 で使う
    static let warmWhite = Color(red: 0xE9 / 255, green: 0xE7 / 255, blue: 0xE2 / 255)
    /// アクセント (夜明けの微光)。#C2A183
    static let dawn = Color(red: 0xC2 / 255, green: 0xA1 / 255, blue: 0x83 / 255)
    /// ヘアライン (区切り線)。rgba(233,231,226,0.08)
    static let hairline = Color.warmWhite.opacity(0.08)
}

/// ヘアラインの区切り線 1 本。List を使わないレイアウトでの行区切りに使う
struct HairlineDivider: View {
    var body: some View {
        Rectangle()
            .fill(Color.hairline)
            .frame(height: 1)
    }
}
