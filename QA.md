...
## 2026-09-17 Shipaton 提出素材の表示確認 (#166)

- 対象コミット: `865f1498c90bab890453b3d37257d87955b1b4f3`。アプリ機能の変更はなく、以下は提出用の画面確認のみ。全機能の最終検証日時は更新しない
- 環境: simtunnel、iPhone 17 / iOS 26.5、英語、Debug ビルド。ビルド・インストール・起動を完了し、撮影後にセッションを停止した
  https://github.com/bannzai/mementomorning/actions/runs/35208023982
- 初回オンボーディングからアラーム・通知を許可し、7:00 の待機ホーム、ジャーナル、月間カレンダー、Seven Mornings、単体共有カード、朝の録画 UI、夜の振り返り、ペイウォールを表示した
- 状態作成は開発者メニューを使用。10日分の回答から当日分を除いた9件の履歴、Seven Mornings、単体カードを撮影。積もる粒は現行実装のホーム背景で、60日分のサンプルから当日分を除いた59件を使用した。夜の振り返りは当日のサンプル文字回答を表示した
- 朝の録画 UI は「動画回答を疑似再現」を有効にして撮影。疑似録画後に回答が保存され、夜の振り返りに「Answered with a video」と再生リンクが出ることも確認した。実機カメラ・実音声の文字起こしは今回の検証対象外
- RevenueCat Test Store の価格表示: 年額 $38.00、月額 $5.00、買い切り $61.50。本番購入・復元、実アラーム発火、通知配信は今回未検証
- 証跡: `shipaton-submission/screenshots/` の9枚。WDA の PNG を 1206×2622 で取得し、縦横比を保って幅1179へ縮小、中央を1179×2556に切り出した。フレーム・見出し・装飾の追加なし。全数の PNG 形式・寸法と、表示内容・文字切れ・秘匿情報の不在を確認した
- 提出チェックは `PASS=43 / FAIL=3` (exit 1)。残る3件は #163 の本番課金確認、許可された動画公開先の URL、動画の一般公開確認のみ
